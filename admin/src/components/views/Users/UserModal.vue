                            ...
                            <form @submit.prevent="onSubmit">
                                <div class="bg-white px-4 pt-5 pb-4">
                                    ...
                                    <CustomInput type="password" class="mb-2" v-model="user.password"
                                        label="Password" />
                                    <select :name="user.name" :value="user.role" required
                                        class="customeInput w-full px-3 py-2 border focus:ring-indigo-500 focus:border-indigo-500"
                                        @change="onChange($event.target.value)">
                                        <option v-for="option of userOptions" :value="option.key">{{ option }}</option>
                                    </select>
                                    <div class="flex items-center mt-2">
                                        <input id="Status" :name="user.name" type="checkbox" :checked="user.status"
                                        @change="changeStatus($event.target.checked)"
                                        class="h-4 w-4 text-indigo-600 focus:ring-indigo-500 border-gray-300 rounded" />
                                    <label for="Status" class="select-none ml-2 block text-sm text-gray-900"> Status </label>
                                    </div>
                                </div>
...
const userOptions = ref(['Admin', 'Staff'])
