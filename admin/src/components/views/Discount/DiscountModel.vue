                        ...
                        <DialogPanel
                            ...
                            <form @submit.prevent="onSubmit">
                                <div class="bg-white px-4 pt-5 pb-4">
                                    <select name="type" v-model="discount.type"
                                        class="input_style w-full px-3 py-2 border focus:ring-indigo-500 focus:border-indigo-500 mb-2">
                                        <option v-if="!discount.type" selected value=''>Discount Type</option>
                                        <option :selected="discount.type=='Overall Price'" value="Overall Price">Overall Price</option>
                                        <option :selected="discount.type=='OnItem'" value="OnItem">OnItem</option>
                                    </select>

                                    <CustomInput type="number" class="mb-2"  v-model="discount.percentage"
                                        label="Percentage" />
                                    <label for="startDate"
                                        class="ml-2 mb-1 text-sm font-semibold text-gray-900/90">Start Date</label>
                                    <input id="startDate" type="date" class="input_style" v-model="discount.startDate"
...
function onSubmit() {
    ...
    if (discount.value.id) {
        store.dispatch('editDiscount', discount.value)
            .then(response => {
                loading.value = false;
                if (response.status === 200) {
                    store.commit('showToast', 'Discount Updated Successfully')
    ...
    {
    if(discount.value.type==='Overall Price'){
     if(!confirm( `Adding Overall Price Discount will delete anyother discount`))
     {
         return
     }
     }
    else {
        if(!confirm( `Adding OnItem Price Discount will delete OverAll discount`))
        {
            return
        }
    }

    store.dispatch('addDiscount', discount.value).then(() => {
        store.commit('showToast', 'Discount Added Successfully')
        store.dispatch('getDiscounts')
        closeModal()
...
onUpdated(() => {
    discount.value = {
        ...
        endDate: props.discount.endDate,
        percentage: props.discount.percentage,
        type: props.discount.type ?  props.discount.type :''
    }
})
