    ...
    <div class="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden">
        <div
            ...
            <div class="flex items-center gap-4">
                <span class="text-sm text-slate-500">Found <span class="font-semibold text-slate-900">{{ cities.data ?
                    cities.data.length : 0
                        }}</span> cities</span>
            </div>

            <div class="relative">
                <input v-model="search" @input="filterCities"
        ...
        <div class="overflow-x-auto">
            <table class="w-full text-left border-collapse">
                ...
                <tbody v-else class="divide-y divide-slate-200">
                    <tr v-for="(city, index) of filteredCities" :key="city.id"
                        class="hover:bg-slate-50 transition-colors">
                        <td class="px-6 py-4 text-sm text-slate-600 font-medium">#{{ city.id }}</td>
                        <td class="px-6 py-4 text-sm text-slate-800 font-medium max-w-[300px] truncate"
                            :title="city.name">
                            {{ city.name }}
                        </td>
                        <td class="px-6 py-4">
                            <img class="w-16 h-16 object-cover rounded-lg" :src="city.image_url" :alt="city.name">
                        </td>
                                ...
                                <transition enter-active-class="transition duration-100 ease-out"
                                    ...
                                    <MenuItems
                                        ...
                                        <div class="px-1 py-1">
                                            ...
                                            <RouterLink :to="{ name: 'app.cities.edit', params: { id: city.id } }"
                                                :class="[
...
const filteredCities = computed(() => {
    ...
    if (!search.value) return cities.value.data
