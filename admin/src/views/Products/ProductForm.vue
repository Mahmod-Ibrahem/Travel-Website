    ...
    <div class="">
        ...
        <form v-else @submit.prevent="onSubmit">
            <div class="bg-white px-4 pt-5 pb-4">
                ...
                <CustomInput class="mb-2" v-model="product.title" :errors="errors.title" label="Tour Title" />
                ...
                <div class="flex flex-col md:flex-row  md:justify-between mt-2 gap-6">
                    <div class="w-full md:w-[49%] flex flex-col">
                        ...
                        <CustomInput type="number" class="mb-2" v-model="product.price_per_person"
                            label="Price Per Person" s :errors="errors.price_per_person" prepend="$" />
                        ...
                        <label for="product-images" class="w-full inline-flex rounded-md border border-gray-300 shadow-sm px-4 py-3 bg-white
               ...
               focus:ring-offset-2 focus:ring-indigo-500 text-xs cursor-pointer">
                        ...
                        <input id="product-images" type="file" class="hidden" accept="image/*" multiple
                            @change="handleProductImages" />
                        <p v-if="errors.tour_images && errors.tour_images[0]"
                    ...
                    <div
                        ...
                        <div class="flex flex-wrap gap-1">
                            <div class="relative group" v-for="(image, index) in product.tour_images" :key="index">
                                <img @click="removeProductImage(index)"
                                    class="w-16 h-16 object-cover cursor-pointer transition-colors hover:brightness-75 flex-shrink-0"
                                    :src="image.path ? image.path : createImageBlob(image)" :alt="product.title" />
                                <div @click="removeProductImage(index)"
