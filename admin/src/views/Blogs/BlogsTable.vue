    ...
    <div class="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden">
        <div
            ...
            <div class="flex items-center gap-4">
                <span class="text-sm text-slate-500">Found <span class="font-semibold text-slate-900">{{ blogs.data ?
                    blogs.data.length : 0
                        }}</span> blogs</span>
            </div>

            <div class="relative">
                <input v-model="search" @input="filterBlogs"
        ...
        <div class="overflow-x-auto">
            <table class="w-full text-left border-collapse">
                ...
                <tbody v-else class="divide-y divide-slate-200">
                    <tr v-for="(blog, index) of filteredBlogs" :key="blog.id"
                        class="hover:bg-slate-50 transition-colors">
                        <td class="px-6 py-4 text-sm text-slate-600 font-medium">#{{ blog.id }}</td>
                        <td class="px-6 py-4 text-sm text-slate-800 font-medium max-w-[300px] truncate"
                            :title="blog.title">
                            {{ blog.title }}
                        </td>
                        <td class="px-6 py-4">
                            <img class="w-16 h-16 object-cover rounded-lg" :src="blog.image_url" :alt="blog.title">
                        </td>
                                ...
                                <transition enter-active-class="transition duration-100 ease-out"
                                    ...
                                    <MenuItems
                                        ...
                                        <div class="px-1 py-1">
                                            ...
                                            <RouterLink :to="{ name: 'app.blogs.edit', params: { id: blog.id } }"
                                                :class="[
...
const filteredBlogs = computed(() => {
    ...
    if (!search.value) return blogs.value.data
