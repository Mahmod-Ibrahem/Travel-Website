...
function onSubmit() {
    loading.value = true
    store.dispatch('createLocationTranslation', location.value)
        .then(response => {
            loading.value = false;
...
onMounted(() => {
    if (route.params.id) {
        loading.value = true;
        store.dispatch("getLocationForTranslation", route.params.id)
            .then((response) => {
                loading.value = false;
