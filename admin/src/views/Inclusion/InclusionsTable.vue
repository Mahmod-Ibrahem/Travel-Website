<template>
    <div class="managmenet_container">
        <div class="flex flex-col  border-b-2 p-2">
            ...
            <span class="mt-4">Found {{ inclusions.total || 0 }} inclusions</span>
        </div>
        <table class="table_tag">
            ...
            <tbody v-else class="table-body">
                <tr v-for="(inclusion, index) of inclusions.data">
                    <td class="border-b p-2 ">{{ inclusion.id }}</td>
                            ...
                            <transition enter-active-class="transition duration-100 ease-out"
                                ...
                                <MenuItems
                                    ...
                                    <div class="px-1 py-1">
                                        ...
                                        <RouterLink :to="{ name: 'app.inclusions.edit', params: { id: inclusion.id } }"
                                            :class="[
        ...
        <div v-if="!inclusionloading" class="justify-between items-center mt-5 md:flex p-3">
            <span>Showing From {{ inclusions.from || 0 }} to {{ inclusions.to || 0 }}</span>
            <nav v-if="inclusions.total > inclusions.per_page"
                class="relative z-0 inline-flex justify-center rounded-md shadow-sm -space-x-px md:flex "
                aria-label="pagination">
                <a v-for="(link, i) of inclusions.links" :key="i" :disabled="!link.url" href="#"
