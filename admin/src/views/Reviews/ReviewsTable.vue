<template>
    <div class="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden">
        ...
        <div class="overflow-x-auto">
            <table class="w-full text-left border-collapse">
                <thead class="bg-slate-50 text-slate-600 uppercase text-xs font-semibold">
                    <tr>
                        ...
                        <TableHeadingCell @click="sortProduct" class="px-6 py-4 border-b border-slate-200"
                            field="tour_id" :sort-field="sortField" :sort-direction="sortDirection">
                            Tour
                        </TableHeadingCell>
                        <TableHeadingCell @click="sortProduct" class="px-6 py-4 border-b border-slate-200" field="title"
                            :sort-field="sortField" :sort-direction="sortDirection">
                            Title
                        </TableHeadingCell>
                        <TableHeadingCell @click="sortProduct" class="px-6 py-4 border-b border-slate-200"
                            field="content" :sort-field="sortField" :sort-direction="sortDirection">
                            Content
                        </TableHeadingCell>
                ...
                <tbody v-else class="divide-y divide-slate-200">
                    <tr v-for="(review, index) of reviews.data" :key="review.id"
                        class="hover:bg-slate-50 transition-colors">
                        <td class="px-6 py-4 text-sm text-slate-600 font-medium">#{{ review.id }}</td>
                        <td class="px-6 py-4 text-sm text-slate-800 font-medium max-w-[200px] truncate"
                            :title="review.tour?.tour_translations[0]?.title">
                            {{ review.tour?.tour_translations[0]?.title ?? 'No Tour' }}
                        </td>
                        <td class="px-6 py-4 text-sm text-slate-600 max-w-[200px] truncate" :title="review.title">
                            {{ review.title }}
                        </td>
                        <td class="px-6 py-4 text-sm text-slate-500 max-w-[300px] truncate" :title="review.content">
                            {{ review.content }}
                        </td>
        ...
        <div v-if="!reviews.loading"
            class="px-6 py-4 border-t border-slate-200 bg-slate-50 flex flex-col md:flex-row justify-between items-center gap-4">
            <span class="text-sm text-slate-500">Showing {{ reviews.from }} to {{ reviews.to }} of {{ reviews.total }}
                results</span>
            <nav v-if="reviews.total > reviews.limit" class="inline-flex rounded-md shadow-sm isolate"
                aria-label="pagination">
                <a v-for="(link, i) of reviews.links" :key="i" href="#" @click.prevent="getForPage($event, link)"
