<div class="flex flex-col items-center w-[380px] mx-3 bg-white rounded-3xl overflow-hidden border relative">
    <div class="p-4 pb-0 w-full h-full">
        @php
            $href = match ($tour->group) {
                'tour-packages' => route('tour-packages.view', [
                    'Tour' => $tour->slug,
                ]),
                'day-tours' => route('day-tours.view', [
                    'Tour' => $tour->slug,
                ]),
            };
        @endphp
        <a href="{{ $href }}">
            <div class="w-full h-60 overflow-hidden rounded-3xl hover:cursor-pointer">
                <img src="{{ $tour->tour_cover }}"
                    class="w-full h-full object-cover  hover:scale-110 transition-all duration-[0.5s]" alt="Tour Card" />
            </div>
        </a>
    </div>
    <div class="flex flex-col items-center p-4 w-full h-full ">
        <a
            class="mt-[6px] mb-[15px] text-[18px] font-bold text-[#1A2B48] hover:cursor-pointer hover:text-main transition-all duration-1000 w-full line-clamp-2 min-h-[50px]">
            {{ $tour->title }}
        </a>

        <p
            class="text-[14px] text-[#4A5568] bordzer-b border-black/10 w-full line-clamp-3 leading-6 min-h-[80px] whitespace-normal">
            {{ $tour->description }}
        </p>
        ...
        <div class="py-[20px] flex justify-between w-full h-full border-b border-black/10 pb-4">
            <div class="flex gap-2">
                ...
                <div class="flex flex-col">
                    ...
                    <span class="text-[#6B7280] text-[14px] font-[400]">{{ $tour->duration }}</span>
                </div>
            </div>
            <div class="flex gap-2">
                <svg width="25px" height="25px" viewBox="0 0 24 24" fill="#3b71fe"
