                ...
                <div class="grid lg:grid-cols-2 grid-cols-1">
                                ...
                                <div class="absolute bottom-0 w-full lg:p-11 p-5">
                                    <div class="bg-white rounded-lg p-6 block">
                                        <a class="flex items-center mb-6">
                                            <svg class="h-7 w-7 text-[#3B82F6]" width="24" height="24"
                    ...
                    <div class="bg-white p-5 lg:p-11 lg:rounded-r-2xl rounded-2xl">
                        ...
                        <input type="text" name="name" required value="{{ old('name') }}"
                            class="w-full h-12 text-gray-600 placeholder-gray-400  shadow-sm bg-transparent text-lg font-normal leading-7 rounded-full border border-gray-200 focus:outline-[#3B82F6] pl-4 mb-2"
                            placeholder="Name">
                        @error('name')
                            <span class="text-red-500 text-sm block mb-8 pl-4">{{ $message }}</span>
                        @else
                            <div class="mb-10"></div>
                        @enderror
                        ...
                        <div class="mb-5">
                            ...
                            <div class="flex">
                                <div class="flex items-center mr-11 gap-1">
                                    <input id="radio-group-1" type="radio" name="communication" value="email" required
                                        {{ old('communication') == 'email' ? 'checked' : '' }}
                                ...
                                <div class="flex items-center gap-1">
                                    <input id="radio-group-2" type="radio" name="communication" value="phone" required
                                        {{ old('communication') == 'phone' ? 'checked' : '' }}
                                        class="    checked:bg-no-repeat checked:bg-center checked:border-indigo-500 checked:bg-indigo-100">
                        ...
                        <textarea id="message" rows="5" name="message" required
                            class="p-2.5 text-sm w-full mb-2 rounded-2xl
                        shadow  font-medium bg-white border border-gray-200 focus:outline-[#3B82F6]"
                            placeholder="Write your message here...">{{ old('message') }}</textarea>
                        @error('message')
                            <span class="text-red-500 text-sm block mb-5 pl-4">{{ $message }}</span>
                        @else
                            <div class="mb-5"></div>
                        @enderror
